...
public class SetTest {
	...
	public static void main(String[] args) {
		...
		// 때문에 수정하는 방법은 해당 자료를 삭제한 후에 추가해 주는 방법을 사용한다.
		 ...
		 * Set형의 데이터들을 Itorator형 객체로 변환하는 메서드 ==> itorator();
		 */
		
		Iterator iterator = hs1.iterator(); // Set데이터를 Iterator로 변환하기
		
		/*
		 * Itorator의 HashNext()메서드
		 *    ==> Itorator의 포인터가 가리키는 곳의 다음번째에 데이터가 있는지 검사한다.
		 *        데이터가 있으면 true, 데이터가 없으면 false로 반환한다.
		 */
		while(iterator.hasNext()) {
			/*
			   Itorater의 next()메서드
			       ==> Itorator의 포인터를 다음번째 위치로 이동시킨 후 그 곳의 데이터를 꺼내서 반환한다. 
			 */
